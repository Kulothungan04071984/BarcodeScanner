...
{
    public class MainForm_New : Form
    {
        // ── Controls ──────────────────────────────────────────────
        ...
        private Label BuildStatLabel(string title, string value, Color bg, Color accent)
        {
            ...
            lbl.Paint += (s, e) =>
            {
                ...
                // rounded rect
                var path = RoundedRect(rc, 10);
                var br   = new SolidBrush(bg);
                g.FillPath(br, path);
                // accent bar
                var barBr = new SolidBrush(accent);
        ...
        private void BuildMain()
        {
            ...
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 72));   // header
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 100));  // input
            layout.RowStyles.Add(new RowStyle(SizeType.Absolute, 36));   // progress
        ...
        private void BuildLogCard()
        {
            ...
            rtbLog = new RichTextBox
            {
                ...
                Size = new Size(panelLogCard.Width - 32, panelLogCard.Height - 66)
            };
        ...
        private static void PaintCard(PaintEventArgs e, Panel panel, Color accent)
        {
            ...
            g.SmoothingMode = SmoothingMode.AntiAlias;
            var path = RoundedRect(rc, 12);
            var br   = new SolidBrush(panel.BackColor);
            g.FillPath(br, path);
            var pen  = new Pen(Color.FromArgb(40, accent.R, accent.G, accent.B), 1);
            g.DrawPath(pen, path);
            // top glow bar
            var grad = new LinearGradientBrush(
        ...
        {
            // Win32 trick to set progress bar colour
            // NativeMethods.SendMessage(pb.Handle, 0x040A, IntPtr.Zero, (IntPtr)0x00FFD200);
           // pb.SetStyle(ControlStyles.UserPaint, true);
        }
    ...
    // ── P/Invoke for progress bar colour ─────────────────────────
